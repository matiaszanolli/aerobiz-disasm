...
- **68K modules**: 801 (extracted to `disasm/modules/68k/<category>/` across 13 categories)
- **Functions identified**: 595 named (0 unnamed), 860 translated, 2,896 unique call targets
- **Translation progress**: ~252,000 bytes translated. All translatable code blocks complete -- only data tables remain as dc.w. All functions named (B-047: 68 from docs/GameCmd, B-047 wave 2-4: 525 from code analysis; B-053: 2 more). All 2,869 jsr abs.l calls symbolized (B-049); 1 RAM jump ($FFF000) remains as dc.w. All 490 jsr (d16,PC) calls symbolized (B-050); 0 dc.w $4EBA remaining. All 337 bsr.w calls symbolized (B-051+B-053); 0 dc.w $6100 remaining in code sections.
